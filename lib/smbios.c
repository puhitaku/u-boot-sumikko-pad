			      ...
			      struct smbios_ctx *ctx)
{
	...
	if (CONFIG_ROM_SIZE >= SMBIOS_ROMSIZE_EXT_MB) {
		t->bios_rom_size = 0xff;
		t->ext_bios_rom_size = CONFIG_ROM_SIZE / (1024 * 1024) &
			SMBIOS_ROMSIZE_EXT_SIZE_MASK;
	} else {
			      ...
			      struct smbios_ctx *ctx)
{
	...
	if (!sysinfo_get_data(ctx->dev, SYSINFO_ID_SMBIOS_CACHE_HANDLE,
		...
		t->l1_cache_handle = *handle? *handle: SMBIOS_CACHE_HANDLE_NONE;
		handle++;
		t->l2_cache_handle = *handle? *handle: SMBIOS_CACHE_HANDLE_NONE;
		handle++;
		t->l3_cache_handle = *handle? *handle: SMBIOS_CACHE_HANDLE_NONE;
	} else {
